...
def check_data():
    with open("./data/data.json", "r") as r:
        config  = json.load(r)
    with open("./CopyFiles/This PC/Local Disk/PythonOS/data.json", "r") as f:
        PyConfig = json.load(f)

    if PyConfig == config:
        return "Config state matched, continuing program."
    elif config == PyConfig:
        return "Config state matched, continuing program."
    else:
        return "Config state isn't matched, 'CopyFiles/This PC/Local Disk/PythonOS/data.json' needs to be changed."
class users():
        ...
        if username is None or password == None:
            ...
            try:
                ...
                with open(f'{path}/data/users/{username}/{username}.json', 'r') as f:
                    r = f.readline()
                return f'Created {username} on Python-OS [ver 1.0]'
    ...
    def change_user(username, new_username, password):
        ...
        try:
            ...
            return f"There is no account associated with '{username}'"
    def login_user(username, password):
        ...
        if os.path.exists(os.path.join(os.getcwd(), f'./data/users/{username}')):
            ...
            if password == passwords:
                try:
                    os.system(f'cd {path}/data/users/{username}/This PC/LocalDisk/PythonOS/ & run.bat')
                    return True
                except KeyError as e:
                    return f'Failed to fetch {e}'
